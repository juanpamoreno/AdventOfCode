const raw = String.raw`"sjdivfriyaaqa\xd2v\"k\"mpcu\"yyu\"en"
"vcqc"
...
let counter = 0
const ENCLOSING_QUOTES_LENGTH = 2
for (let i = 0; i < raw.length; i++) {
  const parsedRaw = raw[i].replace(/\\/g, '*')
  const encodedRaw = raw[i].replace(/"/g, '**').replace(/\\/g, '**')
  counter += ENCLOSING_QUOTES_LENGTH + encodedRaw.length - parsedRaw.length
}
