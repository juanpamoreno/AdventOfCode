...
turn on 222,12 through 856,241`.split('\n').map(instruction => {
  return instruction.replace('turn ', '').replace('through ', '').split(' ')
})

const lightGrid = []

// Initialize lightGrid with all lights off.
...
const operationMap = new Map([
  ['on', (value) => value + 1],
  ['off', (value) => value === 0 ? 0 : value - 1],
  ['toggle', (value) => value + 2]
])

input.forEach(instruction => {
  const start = instruction[1].split(',')
  const end = instruction[2].split(',')
  ...
  for (let i = startX; i <= endX; i++) {
    for (let j = startY; j <= endY; j++) {
      const result = operationMap.get(instruction[0])(Number(lightGrid[i][j]))
      lightGrid[i][j] = result
    }
  }
})

console.log(lightGrid.flat().reduce((acc, currentValue) => acc + currentValue, 0))